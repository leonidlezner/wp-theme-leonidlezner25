<header class="pb-8 md:pb-12 lg:pb-20">
  <div class="bg-color1 lg:py-2" x-data="{ open: false }" @click.outside="open = false" @resize.window="open = false">
    ...
    <div x-cloak class="flex-1 items-center max-lg:flex-col-reverse flex lg:justify-between max-lg:space-y-reverse max-lg:space-y-4 max-lg:bg-color2 max-lg:absolute left-0 right-0 top-[4.2em] z-10 max-lg:pt-4" :class="{ 'max-lg:hidden': !open }">
      ...
      <div>
        <form action="<?php echo home_url(
            "/",
        ); ?>" method="get" class="flex mr-3">
          <input class="focus:ring-white focus:bg-color5 focus:ring-2 text-color1 placeholder:text-color1 rounded-md border-none outline-none bg-color4" name="s" value="<?php the_search_query(); ?>" type="text" placeholder="<?php lang(
    "Search",
); ?>" aria-label="<?php lang("Search"); ?>" />
