...
const devServer = "wordpress.test";
...
add_action("after_setup_theme", function () {
    load_theme_textdomain(
        theme_name,
        get_template_directory() . "/" . resources_path . "/lang",
    );
...
add_action("wp_enqueue_scripts", function () {
    if ($_SERVER["SERVER_NAME"] == devServer) {
        add_filter("https_ssl_verify", "__return_false");
    }

    if (
        $_SERVER["SERVER_NAME"] == devServer &&
        is_array(wp_remote_get("https://localhost:5173/"))
    ) {
        wp_enqueue_script(
            "vite",
        ...
        wp_enqueue_script(
            theme_name,
            "https://localhost:5173/src/js/app.js",
            [],
            null,
            true,
        );
        wp_enqueue_style(
            theme_name,
            "https://localhost:5173/src/css/app.css",
            [],
            "null",
        );
    } else {
        ...
        if (file_exists($manifestFile)) {
            ...
            wp_enqueue_script(
                theme_name,
                get_theme_file_uri(
                    "public/dist/" . $manifest["src/js/app.js"]["file"],
                ),
                [],
                null,
                true,
            ...
            wp_enqueue_style(
                theme_name,
                get_theme_file_uri(
                    "public/dist/" . $manifest["src/css/app.css"]["file"],
                ),
                [],
                null,
            );
